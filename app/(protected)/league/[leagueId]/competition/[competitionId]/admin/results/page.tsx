...
export default function AdminResultsPage() {
  ...
  const competitionId = Array.isArray(params?.competitionId) ? params.competitionId[0] : params?.competitionId ?? "";

  const [competitionName, setCompetitionName] = useState("");
  const [matches, setMatches] = useState<MatchWithEdit[]>([]);
  const [teams, setTeams] = useState<TeamOption[]>([]);
  const [finalResult, setFinalResult] = useState({ first: "", second: "", third: "" });
  ...
  useEffect(() => {
    async function loadData() {
      ...
      const competitionTeamIds = competitionTeamLinks?.map((row) => row.team_id).filter(Boolean) ?? [];

      const matchTeamIds = rawMatches.flatMap((match) =>
        [match.home_team_id, match.away_team_id].filter(Boolean) as string[]
      );

      const allTeamIds = Array.from(new Set([...competitionTeamIds, ...matchTeamIds]));
  ...
  const groupedMatches = useMemo(() => {
    return matches.reduce<Record<string, MatchWithEdit[]>>((acc, match) => {
  ...
  function updateMatch(matchId: string, field: keyof MatchWithEdit, value: string) {
    setStatus(null);
      ...
      <div className="space-y-6">
        <section className="sticky top-0 z-20 rounded-3xl border border-slate-200 bg-white/95 p-4 shadow-sm backdrop-blur">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <h2 className="text-2xl font-semibold text-slate-900">Editovat výsledky</h2>
              <p className="mt-1 text-sm text-slate-600">{competitionName || competitionId}</p>
            </div>
          ...
          {loading ? (
            ...
            <>
              <div className="rounded-3xl border border-slate-200 bg-slate-50 p-5">
                <h3 className="text-lg font-semibold text-slate-900">Reálné TOP 3</h3>
                  ...
                  {(["first", "second", "third"] as const).map((position, index) => (
                    <label key={position} className="block">
                      ...
                      <select
                        ...
                        onChange={(e) => {
                          setStatus(null);
                          setFinalResult((current) => ({ ...current, [position]: e.target.value }));
                        }}
                    ...
                    <div className="mt-4 space-y-3">
                      {stageMatches.map((match) => (
                        <div key={match.id} className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3">
                          <div className="grid gap-3 lg:grid-cols-[150px_1fr_1fr_180px] lg:items-center">
                            ...
                            <div className="grid grid-cols-[1fr_auto_1fr] items-center gap-2">
                              <input
                                ...
                                onChange={(e) => updateMatch(match.id, "home_score", e.target.value)}
                              ...
                              <input
                                ...
                                onChange={(e) => updateMatch(match.id, "away_score", e.target.value)}
                                className="w-full rounded-xl border border-slate-300 bg-white px-3 py-2 text-center text-slate-900 outline-none"
                              />
                            </div>
                          </div>
