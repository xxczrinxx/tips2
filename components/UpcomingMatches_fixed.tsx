  ...
  useEffect(() => {
    async function loadData() {
      ...
      const allTeamIds = Array.from(
        new Set([...matchTeamIds, ...competitionTeamIds]),
      );
      const fetchedTeams =
      ...
      const teamMap = Object.fromEntries(
        fetchedTeams.map((team) => [team.id, team.name]),
      );

      const sortedRawMatches = [...rawMatches].sort((a, b) => {
      ...
      const upcoming = sortedRawMatches.filter(
        (match) => match.kickoff_at && new Date(match.kickoff_at) > now,
      );

      // Extract distinct date keys (YYYY-MM-DD) in order and take first two
      const dateKey = (s: string) =>
        new Date(s).toLocaleDateString("sv-SE", { timeZone: "Europe/Prague" });
      ...
      const filtered =
        distinctDates.length === 0
          ...
          : upcoming
              .filter(
                (m) =>
                  m.kickoff_at && distinctDates.includes(dateKey(m.kickoff_at)),
              )
              .map((match) => ({
                id: match.id,
      ...
      if (competition.tip_top3) {
        const podiumPrediction = await getPodiumPrediction(
          userId,
          competitionId,
        );
    ...
    <div>
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          {competitionName && (
            <p className="mt-1 text-sm text-slate-600">{competitionName}</p>
          )}
        </div>
          ...
          <>
            {top3Enabled && (
              <div className="rounded-3xl border border-slate-200 bg-slate-50 p-5">
                <h4 className="text-md font-semibold text-slate-900">TOP 3</h4>
                              ...
                              <div className="grid grid-cols-1 sm:grid-cols-[50px_300px_80px] items-center justify-start gap-y-2 sm:gap-y-0">
                                ...
                                <div className="grid grid-cols-1 gap-y-2 sm:contents">
                                  <div className="grid w-full sm:w-[300px] grid-cols-[minmax(0,1fr)_20px_minmax(0,1fr)] sm:grid-cols-[140px_20px_140px] items-center gap-1">
                                    <div
                                      className="text-right text-slate-900 font-semibold"
                                    ...
                                    <div
                                      className="text-left text-slate-900 font-semibold"
